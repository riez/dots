...
**Grep hints**: `and.*and`, `or.*or`, `&&.*&&`, `||.*||`, `not.*not`, `!.*!`
...
**Exceptions**: Complex conditions with clear structure and comments explaining the logic.

**Threshold**: Flag when expression requires mental evaluation to understand. Well-commented complex conditions are acceptable.
...
**Grep hints**: `if.*return True.*else.*return False`, `try:.*except:.*pass`, `and do_`
...
**Exceptions**: Exception handling for actual exceptional conditions. Short-circuit for lazy evaluation.

**Threshold**: Flag mechanical anti-patterns only. Intent-preserving variations are style preferences.
...
**Grep hints**: `for i in range(len(`, `+ str(`, `.format(`, callback patterns, `null` checks
...
**Exceptions**: Intentional use of older patterns for compatibility. Performance-critical code avoiding allocations.

**Threshold**: Flag when modern idiom is clearly better AND available in the project's language version. Do not flag style preferences.
...
**Grep hints**: Boolean literals in function calls, magic numbers, unexplained constants
...
**Exceptions**: Well-known constants (0, 1, -1, 100). Boolean in obviously-named function (e.g., setEnabled(true)).

**Threshold**: Flag when meaning requires external lookup. Self-evident code needs no comments.
...
**Grep hints**: Commented blocks, `#if 0`, unreachable branches, unused variables
...
**Exceptions**: Commented code with explanation (debugging aid). Unused params required by interface contract. Public API entry points. Plugin interfaces.

**Threshold**: Flag when code is demonstrably unreachable/unused AND is not a public API entry point, plugin interface, or documented debugging aid.
