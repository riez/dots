...
**Grep hints**: Functions >50 lines, parameter counts >4
...
**Exceptions**: Long functions that do one thing linearly (e.g., state machine, parser). Nesting depth from error handling.

**Threshold**: Flag when function has multiple unrelated responsibilities. Length alone is not a smell.
...
**Grep hints**: `elif.*elif.*elif`, `switch`, `case`, `? :.*? :`
...
**Exceptions**: Exhaustive pattern matching. State machines with explicit states.

**Threshold**: Flag when control flow obscures intent. Explicit branching for documented cases is acceptable.
...
**Grep hints**: `is_.*=`, `has_.*=`, `_flag`, `_state`, multiple boolean assignments
...
**Exceptions**: Single boolean for simple on/off state. Builder pattern flags.

**Threshold**: Flag when flags interact in ways that require mental state tracking. Independent flags are fine.
...
**Grep hints**: `datetime.now`, `time.time`, `os.environ`, `open(`, `requests.`, `http.`
...
**Exceptions**: Entry points that wire dependencies. Test utilities. Scripts meant to run directly.

**Threshold**: Flag when untestable code is in business logic. Infrastructure code at boundaries is expected to have dependencies.
...
**Grep hints**: `except:`, `catch (`, `catch(`, `pass`, `# TODO`, `raise Error(`
...
**Exceptions**: Generic catch at top-level with logging. Intentionally swallowed expected errors with comment.

**Threshold**: Flag when error handling obscures or loses information. Documented catch-all with logging is acceptable.
